...
- **[Analyst-Architect-AI](https://github.com/MatveiV/Analyst-Architect-AI)** — платформа AI-ассистента системного аналитика, покрывающая полный цикл управления требованиями. Основные бизнес-функции и процессы: автоматическое рецензирование ТЗ (анализ полноты, выявление противоречий, риски с вероятностью/влиянием, уточняющие вопросы к постановщику, проверка критериев приёмки) с честными метками `needs_review`/`confidence` и выпуском заключения (DOCX/JSON/CSV); пакетная проверка до 50 ТЗ за запуск; накопление типовых рисков в каталог и уроков (Lessons Learned) для переиспользования в следующих проектах; командная база знаний — загрузка документов, автоиндексация, ответы с цитатами-источниками; генерация требований URS/SRS и ADR по ГОСТ 34 / ISO/IEC 29148; экономика проекта — AI-декомпозиция требований на задачи, оценка трудозатрат по ролям, расчёт CAPEX/OPEX/ROI/payback, план/факт и экспорт бизнес-кейса в DOCX; многоуровневый доступ (admin/analyst/architect) и полный аудит LLM-вызовов (включая локальные Ollama/Qwen в air-gapped-режиме). **Стек:** FastAPI + SQLAlchemy (async) + SQLite/PostgreSQL, React 18 + Vite + TypeScript + Tailwind, 5 LLM-провайдеров, JWT + RBAC, 160 pytest-тестов, Docker Compose (backend + frontend + Kroki + Ollama).
