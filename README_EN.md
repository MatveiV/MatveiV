...
- **[Analyst-Architect-AI](https://github.com/MatveiV/Analyst-Architect-AI)** — an AI-assistant platform for systems analysts covering the full requirements-management cycle. Core business functions and processes: automated spec review (completeness analysis, contradiction detection, risks with probability/impact, clarifying questions for the author, acceptance-criteria checks) with honest `needs_review`/`confidence` labels and deliverable reports (DOCX/JSON/CSV); batch review of up to 50 specs per run; accumulating typical risks into a catalog and Lessons Learned for reuse across projects; a team knowledge base — document upload, auto-indexing, and answers with source citations; URS/SRS/ADR generation per GOST 34 / ISO/IEC 29148; project economics — AI task decomposition, effort estimation by role, CAPEX/OPEX/ROI/payback, plan-vs-actual and business-case export to DOCX; multi-level access (admin/analyst/architect) and full LLM-call auditing (including local Ollama/Qwen in air-gapped mode). **Stack:** FastAPI + SQLAlchemy (async) + SQLite/PostgreSQL, React 18 + Vite + TypeScript + Tailwind, 5 LLM providers, JWT + RBAC, 160 pytest tests, Docker Compose (backend + frontend + Kroki + Ollama).
